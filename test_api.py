...
def test_check_client_id_exists(client):
    ...
    response = client.get(f'/check_client/{existing_client_id}')
    assert response.status_code == 200
    assert response.json is True

def test_check_client_id_not_exists(client):
    non_existing_client_id = 9999
    response = client.get(f'/check_client/{non_existing_client_id}')
    assert response.status_code == 200
...
def test_submit_new_client(client):
    new_client_data = {"some_feature": 123, "another_feature": 456}
...
def test_get_prediction_client_not_found(client):
    non_existing_client_id = 9999
    response = client.post('/prediction', json={'client_id': non_existing_client_id})
    assert response.status_code == 404
    assert response.json == {"error": "Client not found"}
